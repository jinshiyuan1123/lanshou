...
echo "<li style='display:none'><img src='{$url}' style='width:300px;' display='none'><br></li>";
...
class WxpayService
{
    ...
    public function createJsBizPackage($totalFee, $outTradeNo, $orderName, $notifyUrl, $timestamp)
    {
        ...
        include '../includes/modules/payment/weixin.php';
        ...
        // $sql="insert into wx_payment values('','1551','1991','11','111','111','111','111','111','222','33','55')";
        $sql = "insert into wx_payment values('','{$unified['appid']}','{$unified['attach']}','{$unified['body']}','{$unified['mch_id']}','{$unified['nonce_str']}','{$unified['notify_url']}','{$unified['out_trade_no']}','{$unified['spbill_create_ip']}','{$totalFee}','fail','{$unified['trade_type']}','{$timestamp}')";
        // var_dump($sql);
        $data = $db->exec($sql);
